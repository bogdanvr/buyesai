...
class EmailInboundServiceTests(TestCase):
    ...
    def test_updating_same_touch_replaces_lead_event_instead_of_appending_duplicate(self):
        self.deal.delete()
        self.contact.delete()
        self.client_company.email = "company-inbox@example.com"
        self.client_company.save(update_fields=["email"])
        ...
        EmailInboundService.process_raw_email(
            raw_message=self.build_email(
                message_id="email-lead-update-1@example.com",
                subject="Первое письмо",
                body="Первое обращение в лид.",
            ).replace(b"maria@example.com", b"company-inbox@example.com", 1)
        )
        ...
        lead = Lead.objects.get()
        touch = Touch.objects.get(lead=lead)
        touch.summary = "Обновлённый текст касания"
        touch.save(update_fields=["summary", "updated_at"])
...
class CommunicationsApiTests(TestCase):
    ...
    def test_send_message_from_conversation_creates_document_share_link_and_sets_touch_result(self):
    ...
    @override_settings(EMAIL_BACKEND="django.core.mail.backends.locmem.EmailBackend", CRM_PUBLIC_BASE_URL="https://crm.example.com")
    def test_start_conversation_from_deal_creates_document_share_link_and_sets_touch_result(self):
        TouchResult.objects.get_or_create(
            code="proposal_sent",
            defaults={"name": "КП Отправлено", "is_active": True},
        )
...
class DealDocumentDeliveryTests(TestCase):
    def setUp(self):
        self.user = get_user_model().objects.create_user(
            username="deal_document_delivery",
            password="testpass123",
            is_staff=True,
        )
        ...
        self.stage = DealStage.objects.create(
            name="В работе",
            code="in_progress_delivery",
            order=10,
            is_active=True,
            is_final=False,
        )
        self.deal = Deal.objects.create(
            title="Сделка для конвертации",
            client=self.company,
            stage=self.stage,
            owner=self.user,
        )

    @patch("crm_communications.document_delivery.shutil.which")
    @patch("crm_communications.document_delivery.subprocess.run")
    def test_build_deal_document_pdf_bytes_uses_soffice_for_docx(self, subprocess_run_mock, which_mock):
        which_mock.return_value = "/usr/bin/soffice"
        with tempfile.TemporaryDirectory() as tmpdir:
            with override_settings(MEDIA_ROOT=tmpdir):
                document = DealDocument.objects.create(
                    deal=self.deal,
                ...
                def fake_run(command, **kwargs):
                    ...
                    (outdir / f"{source_path.stem}.pdf").write_bytes(b"%PDF-1.4 generated by soffice")
        ...
        self.assertEqual(pdf_name, "Счет № 1235.pdf")
    ...
    def test_public_share_page_and_pdf_download_are_tracked_and_written_to_deal_events(self, build_pdf_mock):
        ...
        message = Message.objects.create(
            conversation=Conversation.objects.create(channel="email", deal=self.deal, client=self.company, status="active"),
            channel="email",
            direction=MessageDirection.OUTGOING,
            status=MessageStatus.SENT,
            client=self.company,
            deal=self.deal,
            subject="Счет",
            body_text="Ссылка на счет",
            sent_at=timezone.now(),
        )
        share = DealDocumentShare.objects.create(document=document, message=message, recipient="email:client@example.com")

        open_response = self.client.get(
            reverse("deal-document-share-page", kwargs={"token": share.token}),
        ...
        self.assertEqual(share.first_open_ip, "203.0.113.10")
        self.assertEqual(share.last_open_user_agent, "TrackedBrowser/1.0")
